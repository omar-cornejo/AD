...
async function convertToHLS(inputFile, channelName, profile = "source") {
  try {
    ...
    const profileConfig =
      ffmpegConfig.profiles[profile] ||
      ffmpegConfig.profiles[ffmpegConfig.defaultProfile];
    ...
    if (profileConfig.copy) {
      ...
      args.push(
        ...
        "libx264",
        "-b:v",
        profileConfig.videoBitrate,
        "-c:a",
        "aac",
        "-b:a",
        profileConfig.audioBitrate,
        "-preset",
        profileConfig.preset,
        "-s",
    ...
    args.push(
      "-start_number",
      String(hlsConfig.startNumber),
      "-hls_time",
      String(hlsConfig.segmentDuration),
      "-hls_list_size",
      String(hlsConfig.listSize),
    ...
    ffmpeg.stderr.on("data", (data) => {
      ...
      if (timeMatch && videoInfo.format.duration) {
        ...
        const progress = Math.floor(
          (currentTime / videoInfo.format.duration) * 100
        );
    ...
    return new Promise((resolve, reject) => {
      ffmpeg.on("close", (code) => {
        ...
        if (code === 0) {
          ...
          console.log(
            `Stream disponible en: /streams/${channelName}/playlist.m3u8\n`
          );
          resolve();
        } else {
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    ...
    console.log(
      "Uso: node convert-to-hls.js <archivo_video> <nombre_canal> [perfil]\n"
    );
